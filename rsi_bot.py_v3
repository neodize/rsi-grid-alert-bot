...
def get_enhanced_grid_setup(coin, rsi):
    ...
    # FIXED: Check if sparkline data exists and has sufficient data
    sparkline_data = coin.get('sparkline_in_7d')
    if not sparkline_data or not sparkline_data.get('price') or len(sparkline_data['price']) < 20:
        # Use fallback volatility calculation based on 24h change
        change_24h = coin.get('price_change_percentage_24h', 0)
        volatility = abs(change_24h) / 100 / 2  # Rough approximation
        # Create synthetic price range based on current price and 24h change
        sparkline = [current_price * (1 + change_24h/100/10 * i) for i in range(-10, 11)]
    else:
        sparkline = sparkline_data['price'][-20:]  # Use more data points
...
def main():
    try:
        ...
        for coin in market_data:
            ...
            # FIXED: Safe sparkline access with fallback
            sparkline_data = coin.get('sparkline_in_7d')
            if not sparkline_data or not sparkline_data.get('price') or len(sparkline_data['price']) < 15:
                # Skip coins without sufficient sparkline data for RSI calculation
                logging.debug(f"Skipping {symbol} - insufficient sparkline data")
                continue
                
            sparkline = sparkline_data['price'][-15:]
            rsi = calc_rsi(sparkline)

            if rsi is None:
                logging.debug(f"Skipping {symbol} - RSI calculation failed")
                continue
