"""
Enhanced Grid Trading Scanner
-----------------------------
* Adds multi‑time‑frame trend, market‑structure and volume confirmation
  to derive a **directional bias** (Long / Short / Neutral) that is far
  better suited to grid‑trading than a pure RSI extreme detector.
* Keeps the original MAIN_TOKENS list exactly intact – these coins are
  always considered even if their liquidity or score is low.
* Preserves the remainder of the prior logic (scoring, alert formatting,
  etc.) so existing integrations continue to work.
"""
