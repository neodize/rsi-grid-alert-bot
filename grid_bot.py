...
def valid(sym):
    u = sym.upper()
    return (u.split("_")[0] not in WRAPPED | STABLE | EXCL and not u.endswith(("UP", "DOWN", "3L", "3S", "5L", "5S")))
...
def score_signal(d):
    return round(
        d["vol"] * 2 + ((200 - d["grids"]) / 200) * 10 + ((1.5 - min(d["spacing"], 1.5)) * 15)
        + (1.5 / max(d["cycle"], 0.1)) * 10, 1
    )

# ── START/STOP MESSAGE FUNCTIONS ───────────────────────
...
def stop_msg(sym, reason, info):
    ...
    now = closes[-1] if closes else (info["low"] + info["high"]) / 2
...
def analyse(sym, interval="5M", limit=400):
    ...
    # Use 5th and 95th percentiles to get a trimmed price range.
    low = np.percentile(closes, 5)
    high = np.percentile(closes, 95)
    ...
    # Ensure the current price is inside the range.
    if px < low:
        low = px
    elif px > high:
        high = px
    ...
    vol = rng / px * 100
    vf = max(0.1, vol + std * 100)  # Safeguard to prevent division by zero
    ...
    pos = (px - low) / rng  # Position of current price within range
